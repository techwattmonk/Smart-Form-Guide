...
export default function EulaPage() {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        const response = await fetch('/api/legal/eula');
        if (response.ok) {
          const data = await response.json();
          setContent(data.content);
        } else {
          setContent('End-User License Agreement content could not be loaded.');
        }
      } catch (error) {
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center h-16">
            ...
            <h1 className="text-xl font-bold text-gray-900">End-User License Agreement</h1>
          </div>
        </div>
      </motion.header>
