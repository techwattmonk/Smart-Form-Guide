...
export default function UtilityUploadPage() {
  ...
  const [keys, setKeys] = useState<string>('Monthly Usage\nBilling Period\nTotal Amount\nRate Schedule\nPeak Demand\nEnergy Charges\nDelivery Charges\nTaxes and Fees');
  ...
  const handleUpload = async () => {
    if (!files.pdf1 || !files.pdf2) {
      setError('Please select both utility bill PDF files');
      return;
    }
          ...
          <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-xl">
            <CardHeader className="text-center">
              ...
              <CardDescription className="text-gray-600">
                Upload two utility bill PDF files for energy analysis and insights extraction
              </CardDescription>
            </CardHeader>
                ...
                <>
                      ...
                      <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-green-400 transition-colors">
                        ...
                        <label htmlFor="pdf1" className="cursor-pointer">
                          ...
                          <p className="text-sm text-gray-600">
                            {files.pdf1 ? files.pdf1.name : 'Click to upload first bill'}
                          </p>
                        </label>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-green-400 transition-colors">
                        ...
                        <label htmlFor="pdf2" className="cursor-pointer">
                          <Upload className="w-8 h-8 text-gray-400 mx-auto mb-2" />
                          <p className="text-sm text-gray-600">
                            {files.pdf2 ? files.pdf2.name : 'Click to upload second bill'}
                          </p>
                        </label>
                      </div>
                    </div>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="keys" className="text-sm font-medium text-gray-700">
                      Energy Data to Extract (one per line)
                    </Label>
                    <textarea
                      id="keys"
                      value={keys}
                      onChange={(e) => setKeys(e.target.value)}
                      placeholder="Enter energy data fields to extract..."
                    ...
                    <p className="text-xs text-gray-500">
                      Pre-filled with common utility bill fields. Modify as needed.
                    </p>
                  </div>
